...
class AddonFileObject():
    ...
    # Whether or not the add-on is enabled in BuddyServers
    self.enabled <bool>
```
...
class ScriptFileObject():
    ...
    # Whether or not the script is enabled in BuddyServers
    self.enabled <bool>
```
...
Fired upon process execution by BuddyServers, not when a player can connect.
...
Fired upon process termination by BuddyServers, not when `/stop` or a crash is logged.
