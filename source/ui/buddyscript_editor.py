...
class BuddyScriptLexer(pygments.lexers.PythonLexer):
...
BuddyScriptLexer.tokens['root'].insert(0, (r'#![\s\S]*#!', Keyword.Header))
BuddyScriptLexer.tokens['root'].insert(-2, (r'(?<!=)(\b(\d+\.?\d*?(?=\s*=[^,)]|\s*\)|\s*,)(?=.*\):))\b)', Number.Float))
BuddyScriptLexer.tokens['root'].insert(-2, (r'(?<!=)(?<!or )(?<!if )(?<!else )(?<!and )(?<!(\+|\-|\&|\||\*|\%|\/|\<|\>) )(\b(\w+(?=\s*=[^,)]|\s*\)|\s*,)(?=.*\):$))\b)', Keyword.Argument))
BuddyScriptLexer.tokens['builtins'].insert(0, (r'(?=\s*?\w+?)(\.?\w*(?=\())(?=.*?$)', Name.Function))
...
proc ::tabdrag::move {win x y} {
  ...
  if { $what eq "" } {
       ...
       set winstate(i,$win) $where
        ...
        window.title(f'{data["app_title"]} - BuddyScript IDE (v{data["buddyscript_version"]})')
        if fullscreen:
