...
def manage_server(name: str, action: str):
    ...
    if action == 'create':
        ...
        return [
            ...
            ("command", "telepath "),
            ("sub_command", "pair "),
            ("info", "to connect from a remote instance of BuddyServers"),
    ...
    try:
        while 'code' not in constants.api_manager.pair_data:
            update_console([
                ...
                ('parameter', '\n\nEnter the IP above into another instance of BuddyServers to continue')
            ])
            time.sleep(1)
            timeout += 1
            if timeout >= total:
...
def update_app(info=False):
    ...
    if info:
        ...
        update_console([("info", response_header), ('parameter', f'Downloading BuddyServers v{constants.update_data["version"].strip()}...')])
...
command_data = {
    ...
    'server': {
        ...
        'sub-commands': {
            ...
            'import': {
                'help': 'imports an existing server from a folder or BuddyServers back-up',
                'params': {
...
if not constants.app_latest:
    content.extend([
        ...
        ("parameter", "(!) An update for BuddyServers is available. Run "),
        ("command", "update "),
        ("sub_command", "info "),
...
title = f"BuddyServers v{constants.app_version}"
...
class ConsolePanel():
    ...
    def build_layout(self):
        title_text = urwid.Text(('title', f"BuddyServers v{constants.app_version} (headless)"), align='center')
...
def run_application():
    ...
    if (constants.is_admin() and not constants.is_docker) and constants.bypass_admin_warning:
        print(f"\n\033[31m> Privilege Warning:  Running BuddyServers as {'administrator' if constants.os_name == 'windows' else 'root'} can expose your system to security vulnerabilities\n\nProceed with caution, this configuration is unsupported\033[0m\n\n< press 'ENTER' to continue >")
        null = input()

    # Raise an error if elevated
    elif (constants.is_admin() and not constants.is_docker) and not constants.bypass_admin_warning:
        print(f"\n\033[31m> Privilege Error:  Running BuddyServers as {'administrator' if constants.os_name == 'windows' else 'root'} can expose your system to security vulnerabilities\n\nPlease restart with standard user privileges to continue\033[0m")
        return False
