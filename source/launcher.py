...
def init_windows_console():
    ...
    app_title = "BuddyServers (console)"
...
def parse_boot_args():
    ...
    try:
        parser = argparse.ArgumentParser(description='CLI options for BuddyServers')

        # Flags
        parser.add_argument(
            '-d', '--debug',
            help = 'execute BuddyServers with verbose console logging',
            action = 'store_true'
        )
