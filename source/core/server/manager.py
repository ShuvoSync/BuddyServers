...
from source.core.constants import (
    ...
    # Constants
    buddyscript_version, os_name, server_ini, command_tmp, color_table,
    valid_config_formats, valid_image_formats, start_script_name
)


# BuddyServers Server Manager API
# ----------------------------------------------- Server Objects -------------------------------------------------------
...
class ServerObject():
    ...
    def __init__(self, _manager: 'ServerManager', server_name: str):
        from source.core.server.buddyscript import ScriptManager, ServerScriptObject
    ...
    def reload_config(self, reload_objects=False, _logging=True, _from_init=False):
        from source.core.server.buddyscript import ScriptManager
        from source.core.server.addons import AddonManager
    ...
    # Writes changes to 'server.properties' and 'buddyservers.ini'
        ...
        def format_log(line, *args):
            ...
            if line:
                ...
                # Server start log
                    ...
                    main_label += '. Type "!help" for BuddyServers commands'
            ...
            with open(script_path, 'r') as f:
                ...
                if os_name == "windows":
                    ...
                    if "OFF" not in str(run('netsh advfirewall show allprofiles | findstr State', shell=True, stdout=PIPE, stderr=PIPE, creationflags=CREATE_NO_WINDOW).stdout):
                        ...
                        if run_proc(f'netsh advfirewall firewall show rule name="BuddyServers java {exec_type}"') == 1:
                            net_test = ctypes.windll.shell32.ShellExecuteW(None, "runas", 'netsh', f'advfirewall firewall add rule name="BuddyServers java {exec_type}" dir=in action=allow enable=yes program="{constants.java_executable[exec_type]}"', None, 0)
                            if net_test == 5:
                        ...
                        def get_latest_crash():
                            ...
                            if not crash_log:
                                ...
                                with open(crash_log, 'w+') as f:
                                    ...
                                    content += "// This report was generated by BuddyServers\n\n"
...
class ServerManager():
    ...
    def load_telepath_servers(self):
        # Possibly run this function before BuddyServers boots, and wait for it to finish loading before showing the UI
...
def calculate_ram(properties):
    ...
    # Attempt to retrieve buddyservers.ini spec first
...
# buddyservers.ini config file function
...
# Creates new buddyservers.ini config file
...
# Creates a new Geyser config with BuddyServers data
