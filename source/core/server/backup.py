    ...
    def _grab_config(self):
        ...
        for cfg in cfg_list:
            ...
            if config:
                ...
                # If config exists, grab version and type information
...
class BackupManager():
    ...
    # Backs up server to the backup directory in buddyservers.ini
...
def convert_date_str(file_name: str):
    ...
    elif file_name.endswith('.bsb'):
        return dt.strptime(file_name.split("__")[1].split(".bsb")[0], "%H.%M %m-%d-%y" + (",%S" if "," in file_name else ""))
    else:
...
def dump_config(server_name: str, new_server=False):
    ...
    # Check buddyservers.ini for info
...
def backup_server(name: str, backup_stats=None, ignore_running=False):
    ...
    if set_lock(name, True, 'save'):
        ...
        try:
            ...
            file_name = f"{name}__{bkup_time}.bsb"
            ...
            while failed:
                ...
                # Create backup
                code = constants.run_proc(f'tar --exclude="*session.lock" -cvf \"{os.path.join(backup_path, f"{name}-bkup.bsb")}\" {"*" if constants.os_name == "windows" else "* .??*"}')
                if code != 0: continue

                failed = False

            if os.path.exists(backup_file): os.remove(backup_file)
            os.rename(os.path.join(backup_path, f"{name}-bkup.bsb"), backup_file)


            # Clear old backups if there's a limit in buddyservers.ini
    ...
    if set_lock(name, True, 'restore'):
        ...
        try:
            ...
            if (len(backup_stats['backup-list']) > 0) and (os.path.basename(backup_name) in [os.path.basename(backup[0]) for backup in backup_stats['backup-list']]):
                ...
                # Rename config to provide xplat support and migrate legacy backups to BuddyServers
            ...
            if ((paths.app_folder not in new_dir) or (new_dir == paths.backups)) and (new_dir != current_dir):
                ...
                if os.access(new_dir, os.W_OK):
                    ...
                    for file in glob(os.path.join(current_dir, f"{name}__*")):
                        ...
                        # Extract config from each match and check the server name just to be sure
                        constants.run_proc(f'tar -xvf "{file}"')  # *buddyservers.ini
                        ...
                        for cfg in configs:
                            ...
                            if config:
                                if config.get('general', 'serverName') == name:
                                    ...
                                    # Update bkupDir with new_dir in the back-ups' config
